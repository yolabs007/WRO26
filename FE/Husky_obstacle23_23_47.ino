// ============================================================
// WRO FE 2026 — OBSTACLE — STEP 4 v17 : FULL RUN (3 laps, no parking)
// v17 (23 Sep): POST-PASS recovery by HEADING (POST_PASS_MODE 2). The v15
//   wall term (+-25) was cancelled by the heading PD: wheels got only +3..+7,
//   car yawed ~10 deg, recovered ~12 cm in 650 mm. Now from RECOVER: full-lock
//   arc to lane +-POST_PASS_DEG (30) toward the centre, hold it until 80% of
//   the offset (side ToFs, slant-corrected by cos(yaw)) is recovered minus
//   the arc-back, then arc back to the lane heading -> normal driving.
//   Ends: target, POST_PASS_MM (1000), corner, new pillar, walls unseen 300 mm.
//   While it runs, a pillar seen in RECOVER is acquired at any heading.
//   Log: columns ppo (offset cm now) and ppd (commanded offset deg) appended.
// v16 (23 Sep, from the v15 23:08 log: red after turn 2 seen late at 81 cm,
//   plan impossible (need 574 > max gain 441), car drove into it, the contact
//   fired a blind-lock turn 3):
//   1. LOGGING: every block HuskyLens returns (col 'blk', id/x/bottom/h/w;...),
//      and the most pillar-like REJECTED block with the reason it failed:
//        N = h < MIN_PILLAR_H     W = wider than PILLAR_W_MAX (entry)
//        E = bottom > ENTRY_BOTTOM_MAX (entry)   P = h/w < PILLAR_ASPECT_MIN (entry)
//        A = gate aspect          S = gate size (too short for its distance)
//        X = tracking x jump      K = tracking bottom jumped back (farther)
//      plus planner state every row: fwd, pass point, theta, need, max gain.
//      'rej' now counts ALL rejections (v14/v15 counted gate + continuity only).
//   2. IMPOSSIBLE PLAN -> BACK: in NONE, a far pillar (fwd >= INFEAS_MIN_FWD_MM)
//      whose plan needs more than max gain + INFEAS_MARGIN_MM: reverse on the
//      lane heading up to INFEAS_BACK_MM (limited by distance since the last
//      HOLD / the corner, minus INFEAS_REAR_GAP_MM; stall stops it), then plan.
//      Once per section.
//   3. PILLAR CORNER WINDOW: from a pillar's HOLD until PILLAR_WIN_MM later no
//      blind-lock corner at all; a corner needs its side open for
//      SIDE_OPEN_CONFIRM readings in a row. Front <= FRONT_BLOCK_CM in the window
//      -> reverse UNSTICK_MM instead of turning (max FRONT_BLOCK_BACKS times).
...
//   rwhy,rid,rx,rbot,rh,rw,fwd,pass,theta,need,maxg,win,blk,ppo,ppd
//
...
// ---------- v16: impossible plan -> back up first ----------
#define INFEAS_ON             1
#define INFEAS_MARGIN_MM     50   // need > max gain + this = impossible (23:08: 574 vs 441)
#define INFEAS_MIN_FWD_MM   450   // only for far pillars (v13 close ones at 290-310 mm passed anyway)
#define INFEAS_BACK_MM      400   // reverse at most this far (23:08 case: 300 -> max gain 566, 400 -> 636 vs need 574)
#define INFEAS_REAR_GAP_MM  250   // keep this much of the distance driven since the last HOLD /
                                  // the corner behind the car (pillar just passed / wall)
#define INFEAS_MIN_BACK_MM  100   // less room than this -> no backup, plan as before

// ---------- v16: pillar corner window ----------
#define PILLAR_WIN_MM       600   // from HOLD: no blind-lock corner, side-open corner needs confirming
#define SIDE_OPEN_CONFIRM     3   // consecutive loops the side must read open inside the window
#define FRONT_BLOCK_CM     20.0   // front this close inside the window -> reverse instead of turning
#define FRONT_BLOCK_BACKS     2   // at most this many such reverses per window, then normal rules
#define FRONT_BLOCK_CONFIRM   5   // ...on this many readings in a row (v13 E T6: F 4-8 for 200 mm while
                                  // the pillar slid past the nose, car still moving = not blocked)
...
#define POST_PASS_MODE        2   // 1 = v15 wall term (clamp POST_PASS_MAX_CORR), 2 = v17 heading arc
#define POST_PASS_DEG        30   // mode 2: heading offset toward the centre during the recovery
...
#define POST_PASS_MM       1000   // window length, from the start of RECOVER (v15: 800)
...
bool  postPassReturning = false; float postPassLastO = 0; long postPassLastValid = 0; float postPassHdgOff = 0; float postPassONow = 0;   // v17
float lastWallCorr = 0;           // logged
// v16 logging / planner / window state
char  rejWhy = '-'; int rejId = 0, rejX = 0, rejBot = 0, rejH = -1, rejW = 0;
char  gateWhy = '-';
String allBlk;
float planNeed = 0, planMaxGain = 0, planFwd = 0;
bool  infeasUsed = false;
long  lastHoldTicks = 0; bool haveHold = false;
bool  pillarWin = false; long pillarWinTicks = 0; int frontBlockBacks = 0; bool cornerHeldLogged = false;
...
void endPostPass(const char* why) {
  if (!postPass) return;
  postPass = false; postPassHdgOff = 0;
...
#if USE_WALL
#if POST_PASS_ON && POST_PASS_MODE == 1
  if (postPass) {
    float o;
...
// v17: heading-based post-pass recovery. true = it drove this loop.
bool postPassDrive(float headingError, float distL, float distR, int speed) {
#if POST_PASS_ON && POST_PASS_MODE == 2
  if (!postPass) return false;
  float c = cos(radians(fabs(headingError)));                          // side beams slant when yawed
  float Lp = (distL < WALL_SEE_CM) ? distL * c : 999.0, Rp = (distR < WALL_SEE_CM) ? distR * c : 999.0;
  float o = 0; bool ok = true;
  if (Lp < WALL_SEE_CM && Rp < WALL_SEE_CM) { o = (Lp - Rp) * 0.5; postPassLaneW = Lp + Rp; }
  else if (postPassLaneW > 0 && Lp < WALL_SEE_CM) o = Lp - postPassLaneW * 0.5;
  else if (postPassLaneW > 0 && Rp < WALL_SEE_CM) o = postPassLaneW * 0.5 - Rp;
  else ok = wallOffsetCm(Lp, Rp, o);
  if (ok) { postPassLastO = o; postPassLastValid = encoderTicks; }
  else {
    if (!postPassHaveO0) return false;                                 // nothing measured yet: normal driving
    if (mmSince(postPassLastValid) > 300) { endPostPass("walls not seen"); return false; }
    o = postPassLastO;
  }
  postPassONow = o;
  if (!postPassHaveO0) {
    postPassO0 = o; postPassHaveO0 = true;
    if (fabs(o) < POST_PASS_MIN_CM) { endPostPass("already near the line"); return false; }
    logLine("  POST-PASS start: offset " + String(o, 0) + " cm -> target " + String((1.0 - POST_PASS_FRACTION) * o, 0) + " cm, heading lane " + (o < 0 ? "+" : "-") + String(POST_PASS_DEG) + " deg");
  }
  float oT   = (1.0 - POST_PASS_FRACTION) * postPassO0;
  float sgn  = (postPassO0 < 0) ? 1.0 : -1.0;                          // + = move right
  float togo = sgn * (oT - o);                                         // cm still to go toward the centre
  float arcBack = TURN_R_MM * (1.0 - cos(radians(POST_PASS_DEG))) / 10.0;   // cm the arc back still gains
  if (!postPassReturning && togo <= arcBack) {
    postPassReturning = true;
    logLine("  POST-PASS: offset " + String(o, 0) + " cm, arc back to lane  mm=" + String(mmSince(0), 0));
  }
  postPassHdgOff = postPassReturning ? 0 : sgn * POST_PASS_DEG;
  float hdg = target_heading + postPassHdgOff;
  if (hdg >= 360) hdg -= 360; if (hdg < 0) hdg += 360;
  float current = target_heading - headingError; if (current >= 360) current -= 360; if (current < 0) current += 360;
  driveArcToHeading(hdg, current, speed);                              // full lock until within SWERVE_BLEND_DEG
  if (postPassReturning && fabs(headingError) < 5) endPostPass("target reached");
  return true;
#else
  return false;
#endif
...
void driveStraightStep(float headingError, float distL, float distR, int speed) {
  if (postPassDrive(headingError, distL, distR, speed)) return;        // v17
...
int gateClass(int id, int xc, int yc, int w, int h) {
  ...
  if ((float)h < aspMin * (float)w) { gateWhy = 'A'; return 0; }
  float frac = (id == GREEN_ID) ? PIL_H_MIN_FRAC_G : PIL_H_MIN_FRAC;
  if ((float)h < frac * (PIL_H_SLOPE * bottom + PIL_H_OFS)) { gateWhy = 'S'; return 0; }
  return 2;
#else
  return 2;
#endif
...
// v16: remember the most pillar-like (tallest) rejected block and why
void noteRej(char why, const HUSKYLENSResult &r) {
  pRej++;
  if (r.height > rejH) { rejH = r.height; rejWhy = why; rejId = r.ID; rejX = r.xCenter; rejBot = r.yCenter + r.height / 2; rejW = r.width; }
}
bool readPillar(bool tracking) {
  pSeen = false; pCount = 0; pRej = 0;
  rejWhy = '-'; rejH = -1; rejId = rejX = rejBot = rejW = 0; allBlk = "";   // v16
  ...
  while (husky.available()) {
    ...
    if (allBlk.length() < 110) {                                             // v16: every block, as returned
      if (allBlk.length()) allBlk += ';';
      allBlk += String(r.ID) + "/" + String(r.xCenter) + "/" + String(r.yCenter + r.height / 2) + "/" + String(r.height) + "/" + String(r.width);
    }
    if (r.ID != RED_ID && r.ID != GREEN_ID) continue;
    if (r.height < MIN_PILLAR_H) { noteRej('N', r); continue; }
    pCount++;
    if (!tracking) {
      if (r.width > PILLAR_W_MAX) { noteRej('W', r); continue; }                                   // line
      if (r.yCenter + r.height / 2 > ENTRY_BOTTOM_MAX) { noteRej('E', r); continue; }               // too close to be a new pillar
      if ((float)r.height / (float)(r.width > 1 ? r.width : 1) < PILLAR_ASPECT_MIN) { noteRej('P', r); continue; }
      int g = gateClass(r.ID, r.xCenter, r.yCenter, r.width, r.height);        // v14: size for its distance
      if (g == 0) { noteRej(gateWhy, r); continue; }
      ...
      if (dx > dxLim) { noteRej('X', r); continue; }                          // v14: jumped sideways
      if (r.yCenter + r.height / 2 < refBottom - TRACK_MAX_BACK) { noteRej('K', r); continue; }   // v14: jumped farther
#else
...
float planSwerve(float offRightDeg, bool logIt) {
  ...
  planNeed = need; planFwd = fwd; planMaxGain = lateralGain(THETA_MAX, fwd, yawNow);   // v16
  ...
  if (logIt) logLine("  plan: d=" + String(d_mm, 0) + "mm bearing=" + String(bearL, 1) + " pillarLat=" + String(pilLat, 0) +
                     ...
                     " (max gain " + String(planMaxGain, 0) + (pMeas ? "" : ", clipped block") +
                     (need > planMaxGain + INFEAS_MARGIN_MM ? ", IMPOSSIBLE" : "") + ")");
  return sgn * th;
}
...
// v16: far pillar whose plan cannot be flown -> how far may we reverse first? (0 = don't)
float infeasBackMM(float offRightDeg) {
#if INFEAS_ON && SWERVE_MODE == 2
  if (infeasUsed) return 0;
  float keepPass = planPassMM;
  planSwerve(offRightDeg, false);                                      // fills planNeed / planMaxGain / planFwd
  planPassMM = keepPass;
  if (planFwd < INFEAS_MIN_FWD_MM) return 0;
  if (planNeed <= planMaxGain + INFEAS_MARGIN_MM) return 0;
  float room = mmSince(legStartTicks);                                 // since the corner
  if (haveHold) room = min(room, mmSince(lastHoldTicks));              // since the pillar just passed
  float back = min((float)INFEAS_BACK_MM, room - INFEAS_REAR_GAP_MM);
  if (back < INFEAS_MIN_BACK_MM) {
    logLine("  IMPOSSIBLE plan (need " + String(planNeed, 0) + " > max " + String(planMaxGain, 0) + ") but no room behind (" + String(room, 0) + " mm) -> plan anyway");
    infeasUsed = true; return 0;
  }
  return back;
#else
  return 0;
...
bool decideTurn(float distF, float distL, float distR) {
  ...
  if (pillarWin) {                                                     // v16: just after a pillar
    static int openCount = 0;
    bool open = (lockedTurnDirection ==  1) ? rightOpen :
                (lockedTurnDirection == -1) ? leftOpen : (rightOpen != leftOpen);
    openCount = open ? openCount + 1 : 0;
    if (openCount < SIDE_OPEN_CONFIRM) {
      if (!cornerHeldLogged) { logLine("  corner held (pillar window): F=" + String(distF, 0) + " L=" + String(distL, 0) + " R=" + String(distR, 0) + " open x" + String(openCount)); cornerHeldLogged = true; }
      return false;                                                    // never a blind-lock corner here
    }
    openCount = 0;
    logLine("  corner in pillar window: side open " + String(SIDE_OPEN_CONFIRM) + "x confirmed");
    if (lockedTurnDirection == 0) { lockedTurnDirection = rightOpen ? 1 : -1; logLine(lockedTurnDirection == 1 ? "LOCKED: CLOCKWISE" : "LOCKED: ANTI-CLOCKWISE"); }
    return true;
  }
...
void endTurn(const char* why, float distL, float distR) {
  ...
  infeasUsed = false; haveHold = false; pillarWin = false;             // v16
#if SCAN_BACK
  ...
  switch (pState) {
    case P_NONE:
      ...
      if (seen) {
        ...
        { float b = infeasBackMM(-headingError);                           // v16: far and impossible
          if (b > 0) {
            infeasUsed = true;
            startBack(b, false, (String("IMPOSSIBLE ") + (pColor == RED_ID ? "RED" : "GREEN") + " x=" + String(pX / (float)FRAME_W, 2) + " bottom=" + String(pY + pH / 2) +
                                 " need " + String(planNeed, 0) + " > max " + String(planMaxGain, 0) + " at fwd " + String(planFwd, 0)).c_str());
            return true;
          } }
    ...
    case P_COMMIT:
      ...
      // Camera losing it (yaw takes it out of frame) does NOT end COMMIT.
      {
        ...
        if (fwdCarMM >= pillarFwdMM || capped) {
          ...
          lastHoldTicks = encoderTicks; haveHold = true;                                      // v16
          pillarWin = true; pillarWinTicks = encoderTicks; frontBlockBacks = 0; cornerHeldLogged = false;
    ...
    case P_HOLD:
      ...
      if (phaseDone(PASS_HOLD_MS, PASS_HOLD_MM)) { returnTargetLat = latCarMM * (1.0 - RETURN_FRACTION); enterPhase(P_RECOVER, "RECOVER"); logLine("     return target carLat=" + String(returnTargetLat, 0) + " from " + String(latCarMM, 0));
        ...
        postPassReturning = false; postPassLastValid = encoderTicks; postPassHdgOff = 0;          // v17
#endif
      }
      return true;
#elif SWERVE_MODE
    ...
    case P_BACK: {                                                         // v14
      ...
      bool clean   = false;
      if (!backForPillar) { pSeen = false; pMeas = false; pRej = 0; rejWhy = '-'; rejH = -1; rejId = rejX = rejBot = rejW = 0; allBlk = ""; }   // camera not read
      if (backForPillar) {
    ...
    case P_RECOVER:
#if SWERVE_MODE == 2
      {
        ...
        if (nextSeen && (fabs(headingError) <= ACQ_STRAIGHT_DEG || postPass)) { acquirePillar(-headingError, pillarCount + 1); return true; }   // v17: any heading while recovering
#if LANE_RETURN
...
void setup() {
  ...
  logLine(String("STEP 4 v17 FULL RUN ready (MOTOR_ON=") + MOTOR_ON + ", SWERVE_MODE=" + SWERVE_MODE + ", GATE=" + GATE_ON + ", BACKUP=" + BACKUP_ON + ", UNSTICK=" + UNSTICK_ON + ", POST_PASS=" + POST_PASS_ON + "/mode" + POST_PASS_MODE + ", INFEAS=" + INFEAS_ON + "). Press START."); logFlush();
  ...
  logLine("t_ms,mm,state,F,L,R,hdg,tgt,h_err,servo,col,x,x_frac,y,bottom,h,w,blocks,carLat,turn,meas,rej,wc,rwhy,rid,rx,rbot,rh,rw,fwd,pass,theta,need,maxg,win,blk,ppo,ppd");
}
...
void loop() {
  ...
  if (pillarWin && (mmSince(pillarWinTicks) > PILLAR_WIN_MM || pState == P_COMMIT)) pillarWin = false;   // v16
  ...
  logLine(String(millis() - runStartTime) + "," + String(mmSince(0), 0) + "," + pStateName() + "," + String(distF, 0) + "," + String(distL, 0) + "," + String(distR, 0) + "," +
          ...
          String(rejWhy) + "," + String(rejId) + "," + String(rejX) + "," + String(rejBot) + "," + String(rejH < 0 ? 0 : rejH) + "," + String(rejW) + "," +
          String(fwdCarMM, 0) + "," + String(pState == P_COMMIT ? pillarFwdMM : 0, 0) + "," + String(pState == P_COMMIT ? planTheta : 0, 0) + "," +
          String(planNeed, 0) + "," + String(planMaxGain, 0) + "," + (pillarWin ? "W" : "-") + "," + allBlk + "," +
          (postPass ? String(postPassONow, 0) : String("-")) + "," + (postPass ? String(postPassHdgOff, 0) : String("-")));

#if MOTOR_ON
  // v16: front blocked right after a pillar -> reverse, never turn
  static int frontBlockCount = 0;
  if (pillarWin && !isTurning && (pState == P_NONE || pState == P_RECOVER) && distF > 2.0 && distF <= FRONT_BLOCK_CM) frontBlockCount++;
  else frontBlockCount = 0;
  if (frontBlockCount >= FRONT_BLOCK_CONFIRM) {
    frontBlockCount = 0;
    if (frontBlockBacks < FRONT_BLOCK_BACKS) {
      frontBlockBacks++;
      logLine("  FRONT BLOCKED in pillar window: F=" + String(distF, 0) + " L=" + String(distL, 0) + " R=" + String(distR, 0) + " h_err=" + String(headingError, 1) + " (" + String(frontBlockBacks) + "/" + String(FRONT_BLOCK_BACKS) + ")");
      startBack(UNSTICK_MM, false, "front blocked after pillar");
    } else { pillarWin = false; logLine("  pillar window closed: front still blocked, normal corner rules"); }
  }
#endif
